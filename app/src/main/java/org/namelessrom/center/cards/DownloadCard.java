...
public class DownloadCard extends Card implements DownloadHelper.DownloadCallback {
    ...
    private Updater.PackageInfo[] mDownloading;
    private int mDownloadProgress = -1;

    public DownloadCard(Context context, AttributeSet attrs, Updater.PackageInfo[] infos,
        ...
        if (infos == null && savedInstanceState != null) {
            infos = (Updater.PackageInfo[]) savedInstanceState.getSerializable(DOWNLOADING);
    ...
    public void setInitialInfos(Updater.PackageInfo[] infos) {
    ...
    private void setInfos(Updater.PackageInfo[] infos, int progress) {
        Context context = getContext();
        String names = "";
        for (int i = 0; infos != null && i < infos.length; i++) {
            names += infos[i].getFilename() + "\n";
