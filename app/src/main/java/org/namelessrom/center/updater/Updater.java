...
public abstract class Updater implements Response.Listener<JSONArray>, Response.ErrorListener {
    ...
    protected void nextServerCheck() {
        ...
        final JsonArrayRequest req = new JsonArrayRequest(buildUrl(), this, this);
        mQueue.add(req);
    }

    protected String buildUrl() {
        final int updateChannel = new SettingsHelper(getContext()).getUpdateChannel();
        final String url;
        final String query;
        switch (updateChannel) {
            default:
            case SettingsHelper.UPDATE_CHANNEL_ALL:
                url = Updater.CHANNEL_ALL;
                query = "";
                break;
            case SettingsHelper.UPDATE_CHANNEL_NIGHTLY:
                url = Updater.CHANNEL_NIGHTLY;
                query = "";
                break;
            case SettingsHelper.UPDATE_CHANNEL_WEEKLY:
                url = Updater.CHANNEL_WEEKLY;
                query = "?display=full";
                break;
        }
        return mServer.getUrl(getDevice(), url, query);
    }
